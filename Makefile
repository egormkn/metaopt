...
SRC_METAOPT_MODEL=Model.cpp Metabolite.cpp Reaction.cpp Coupling.cpp Precision.cpp
...
$(BIN_DIR)/$(LIBRARY) : $(OBJECTS)
	mkdir -p $(BIN_DIR)
	$(LD) -o $@ $(LDFLAGS) $(OBJECTS) -L$(SCIP_LIB) $(LSBML) $(LMATLAB) -Xlinker -rpath=$(SCIP_LIB):$(RSBML):$(RMATLAB) $(libSBML) $(libMATLAB) -lscip -lobjscip -llpispx -lnlpi.cppad -lsoplex -lzimpl -lz -lgmpxx -lreadline -lncurses -lm
