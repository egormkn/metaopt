...
#include "model/text/TextLoader.h"
...
namespace metaopt {
    ...
    int fba(const TextLoader& loader) {
...
*/
     ...
     */
    int tfba(const TextLoader& loader, int nargout) {
        ModelPtr model = loader.getModel();
...
*/
    ...
    void convert_fva_result(metaopt::ModelPtr &model, const metaopt::TextLoader &loader,
 ...
 */
    int tfva(const TextLoader& loader, int nargout, double timeout) {
        ModelPtr model = loader.getModel();
...
int main(int argc, const char *argv[]) {
    ...
    try {
        ...
        std::string line;
        std::vector<std::tuple<int, int, double>> stoichiometry;
        std::vector<std::tuple<double, double, double>> limits;

        std::ifstream s_file(metabolites);
        if (!s_file) {
        ...
        double value;
        int species = 0;
        while (s_file >> row >> col >> value) {
            species = max(species, row);
            stoichiometry.emplace_back(row, col, value);
        }
        s_file.close();


        std::ifstream r_file(reactions);
        if (!r_file) {
            throw std::runtime_error("Unable to open file");
        }
        std::getline(r_file, line);
        double low, high, weight;
        while (r_file >> low >> high >> weight) {
            limits.emplace_back(low, high, weight);
        }
        r_file.close();

        metaopt::TextLoader loader;
        loader.load(stoichiometry, limits, species);

        if (solver == "fba") {
            metaopt::fba(loader);
        } else if (solver == "tfba") {
            metaopt::tfba(loader, 2);
        } else if (solver == "fva") {
            cout << "Not implemented" << endl; // TODO
        } else if (solver == "tfva") {
            metaopt::tfva(loader, 2, -1.0);
        }
